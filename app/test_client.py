    ...
    def run(host='localhost', port=8080): # ¡Puerto CAMBIADO a 8080 para conectar a Envoy!
        # Nota: Un cliente gRPC nativo como este se conectará al cluster de Envoy
        # que espera HTTP/2. gRPC-Web es para clientes de navegador.
        # Sin embargo, esta configuración de Envoy con http2_protocol_options
        # en el listener y el cluster también puede manejar solicitudes gRPC nativas.
        channel = grpc.insecure_channel(f'{host}:{port}')
        stub = productos_pb2_grpc.ProductosServiceStub(channel)

        print(f"--- Probando Listar Productos en {host}:{port} (a través de Envoy) ---")
        try:
    ...
    if __name__ == "__main__":
        ...
        if len(sys.argv) == 3:
            ...
            run() # Conecta a localhost:8080 por defecto (a través de Envoy)
